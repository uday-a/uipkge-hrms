 ...
 *
 * The HRMS mock surface is synchronous module imports, so every page
 * has its data "ready" the moment it renders. That doesn't match what
 * a real consumer will experience -- their KPI tiles, charts, and
 * tables get their data via `useFetch`/`useAsyncData` and have a
 * visible loading window. Skeletons exist for that window.
 *
 * To prove the skeleton story works end-to-end (criterion #6 in
 * `docs/template-design.md`), pages gate their loaded-state markup on
 * this composable and render `<Skeleton>` placeholders in the meantime.
