 ...
 * Mock sign-in page.
 *
 * Everything here is theatre. The form takes any input and lands the
 * user on /dashboard. The "Continue as…" row is the real entry point
 * for the demo -- it bypasses the form, sets the persona, and routes.
 *
 * Rendered without the dashboard layout so the centred card has the
 * whole viewport. `layout: false` is checked first (see app.vue chain).
 */
import { ref } from 'vue'
...
// Pre-fill the email so the form looks "remembered" -- common UX in
// real sign-in pages and removes a fiddly step for the demo viewer.
const email = ref('marcus@uipkge.dev')
const password = ref('')
const remember = ref(true)

async function handleSubmit() {
  // The whole template is mock auth -- no validation, no API. Toast
  // the persona we are about to land as so the user knows the demo
  // is leaning on a fixed identity.
  toast.success('Signed in as Marcus Rivera')
...
// Order here drives the row layout below. Keeping the data inline
// instead of in a /lib file because there are exactly three personas
// and this is the only place that renders them.
const personas: Array<{ key: Persona; label: string; tagline: string }> = [
  { key: 'admin', label: 'Admin', tagline: 'See everything' },
  { key: 'manager', label: 'Manager', tagline: 'Team + approvals' },
  { key: 'employee', label: 'Employee', tagline: 'Just my stuff' },
]
</script>
    ...
    <div class="w-full max-w-md space-y-4">
      <Card class="rounded-2xl border-2 shadow-xl">
        <CardContent class="space-y-6 p-6 sm:p-8">
          <!-- Header: matches the Sidebar02 "h" tile so the brand reads
               the same whether you arrive logged-out or logged-in. -->
          <div class="space-y-3">
            <div class="flex items-center gap-2">
              <div class="bg-primary text-primary-foreground flex size-9 items-center justify-center rounded-md text-lg font-bold">
                h
              </div>
              <div class="flex flex-col leading-tight">
                <span class="text-sm font-semibold tracking-tight">uipkge HRMS</span>
                <span class="text-muted-foreground text-xs">Reference template · v2</span>
              </div>
            </div>
            <div class="space-y-1 pt-2">
              <h1 class="text-2xl font-bold tracking-tight">Welcome back</h1>
              <p class="text-muted-foreground text-sm">Sign in to your uipkge HRMS workspace.</p>
            </div>
          </div>

          <!-- Mock form. @submit.prevent so the page never reloads. -->
            ...
            <div class="space-y-1.5">
              <div class="flex items-center justify-between">
                ...
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  class="h-auto px-1 text-xs font-medium"
                  @click="fakeForgot"
                >
                  Forgot password?
                </Button>
          ...
          <div class="relative">
            <Separator />
            <span class="bg-card text-muted-foreground absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 px-2 text-xs font-medium uppercase tracking-wide">
              Or continue with
            </span>
          </div>
          ...
          <div class="grid grid-cols-2 gap-2">
            <Button type="button" variant="outline" class="w-full" @click="continueWithSso('Google')">
              <!-- Inline SVG keeps the brand-glyph crisp without adding an
                   icon dependency. Three-colour Google mark, simplified. -->
          ...
          <!-- "Continue as" demo row -- the actual entry point for the
               template. Three buttons sit in a single divided row so it
               reads as one control, not three separate CTAs. -->
          <div class="space-y-2">
            ...
            <p class="text-muted-foreground text-xs">
              Skip auth and jump straight in — this is a mock template.
            </p>
      ...
      <p class="text-muted-foreground text-center text-sm">
        ...
        <Button variant="link" class="h-auto p-0 text-sm font-medium" @click="fakeSignUp">
          Sign up
        </Button>
      </p>
    </div>
